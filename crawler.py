...
class Crawler(object):
    ...
    def _downimage(self,page):
        ...
        for link in soup.find_all('img'):
            hrefs.append(link.get('original'))
        hrefs = [ i for i in hrefs if i]
        for imgurl in hrefs[:-1]:
            urllib.urlretrieve(imgurl, '%s.%s' % (str(x), imgType))
            x =x+1
